...
const SplashScreen = ({ onFinished }: { onFinished: () => void }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] bg-[#1c324e] flex items-center justify-center transition-all duration-700 ease-in-out ${step === 3 ? 'opacity-0 pointer-events-none scale-110 blur-xl' : 'opacity-100'
        }`}
    >
      <div className="relative flex flex-col items-center">
        ...
        <div className="overflow-hidden h-16 flex items-center justify-center relative">
          <h1
            className={`font-serif text-5xl text-white font-bold tracking-wider transform transition-all duration-700 ${step >= 2 ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'
              }`}
          >
            Firefly
          </h1>
        ...
        <div
          className={`font-sans text-sm font-bold text-firefly-yellow tracking-[0.4em] uppercase mt-2 transition-all duration-700 delay-100 ${step >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}
        >
          Creative Solutions
        </div>
