...
import { ArrowRight, Sparkles, Play, Pause, Heart, MessageSquare, Send, Bookmark } from 'lucide-react';
                ...
                >
                  ...
                  <div className={`absolute inset-0 flex items-center justify-center bg-black/0 ${!isPlaying ? 'animate-pulse' : ''}`}>
                    {!isPlaying ? (
                      ...
                      isInteracting && <Pause className="w-12 h-12 text-white/80" fill="currentColor" />
                    )}
                  </div>
                  {/* Instagram Reels-like Icons */}
                  <div className="absolute bottom-4 left-4 flex flex-col items-center space-y-4 z-20">
                    <div
                      className={`flex flex-col items-center transition-all duration-300 cursor-pointer ${isLiked ? 'text-red-500' : 'text-white/60 hover:text-white'
                        }`}
                      onClick={(e) => {
                        e.stopPropagation();
                        setIsLiked(!isLiked);
                    ...
                    >
                      <Heart className="w-6 h-6 transition-transform duration-200 hover:scale-110" fill="currentColor" />
                      <span className="text-xs">{likes >= 1000 ? (likes / 1000).toFixed(1) + 'K' : likes}</span>
                    </div>
                    <div className="flex flex-col items-center text-white/60 hover:text-white transition-colors cursor-pointer" onClick={() => setComments(comments + 1)}>
                      <MessageSquare className="w-6 h-6" fill="currentColor" />
                      <span className="text-xs">{comments}</span>
                    </div>
                    <div className="flex flex-col items-center text-white/60 hover:text-white transition-colors cursor-pointer" onClick={() => setShares(shares + 1)}>
          ...
          <div className="w-full lg:w-1/2 text-center lg:text-left pt-10 lg:pt-0">
            ...
            <Reveal delay={1700} width="100%">
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                ...
                <button
                  className="w-full sm:w-auto px-8 py-4 bg-firefly-yellow hover:bg-white text-firefly-dark font-bold rounded-lg transition-all duration-300 flex items-center justify-center gap-2 text-lg overflow-hidden relative"
                  onClick={onShowReel}
                >
